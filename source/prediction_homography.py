...
def get_perspecitve_transform(calib_path, img, img_points, obj_points, PLT_VIEW=False, visualize=False):
    """
    Args
    ===
    * calib_path (str) : calibration result json file path
    * img (np.ndarray) : sample image
    * img_points (np.ndarray) : sample image points to use homography function
    * obj_points (np.ndarray) : sample object points to use homography function
    * PLT_VIEW (bool) : whether show the undistort image
    * visualize (bool) : check pairs of points
    ...
    calibration_jsonfile_path = os.path.join(calib_path)
...
if __name__ =="__main__":
    PLT_VIEW = False
    visualize = False
    camera_height = -16.5 # [cm]

    # load sample image
    img_file_path="./source/images/img_1.jpg"
    img = cv2.imread(img_file_path, cv2.IMREAD_ANYCOLOR)

    # point lists to find homography
    img_points = np.array([
                        [170, 426], # bottom left
    ...
    bbox_point = np.array([center_point[0], center_point[1], camera_height], dtype=np.float32)

    # perspective transformation H
    homography = get_perspecitve_transform("./calibration.json", img, img_points, obj_points)
    
    # distance inference
    estimation = np.dot(homography, bbox_point)
