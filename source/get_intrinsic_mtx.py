...
def get_intrinsic_infp(basedir = None, CHESSBOARD_WIDTH = 9, CHESSBOARD_HEIGHT = 6):
    base_dir = "./"
    ...
    ret, camera_matrix, extrinsic, rvecs, tvecs = cv2.calibrateCamera(
                                    object_points, image_points, gray.shape[::-1], None, None  
                                    )

    for rvec, tvec, op, ip in zip(rvecs, tvecs, object_points, image_points):
        imagePoints, jacobian = cv2.projectPoints(op, rvec, tvec, camera_matrix, extrinsic)
